...
def main():
    ...
    # Use the context manager to handle DB setup and teardown
    with DB(config_file=config_file) as db:
        ...
        # Queued Write
        if not db.worker.is_running:  # Start worker if not running
            db.start()
        db.write_message(
        ...
        # Direct Write and Verify
        db.write_message(
            "dynamo_key1", {"data": "dynamo_direct_write"}, backend="dynamodb"
        )
        ...
        # Queued Write
        if not db.worker.is_running:
            db.start()
        db.write_message(
            "dynamo_key2",
            {"data": "dynamo_queued_write"},
            backend="dynamodb",
            queue=True,
        )
        ...
        # ================================
        # Stress Test and Concurrent Writes
        # ================================
        print("\n== Stress and Concurrent Tests ==")
        stress_test(db, backend="local", num_writes=50)
        concurrent_writes(db, backend="local", prefix="concurrent_local")
